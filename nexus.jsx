(function() {
  ...
  async function getRoom() {
    ...
    if (window.WebsimSocket) {
      try {
        const instance = new WebsimSocket();
  ...
  window.NexusAPI = {
    ...
    async isOwner(project, currentUserIdentifier) {
      ...
      try {
        const identifier = (currentUserIdentifier || "").toString().toLowerCase();
        if (identifier === "@random_user_1") return true;
        const author = (project.author || "").toString().toLowerCase();
    ...
    async createProject(payload) {
      ...
      if (typeof newProject.public === "undefined") newProject.public = true;
  ...
  (async function ensureInit() {
    try {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i] && (arr[i].previewImage || arr[i].preview_image)) {
      ...
      (async function syncLocalToRemote() {
        try {
          ...
          const remoteKeys = new Set((remoteList || []).map((r) => r.id ? "id:" + r.id : "t:" + ((r.title || "") + "|" + (r.created_at || ""))));
          for (const localP of arr || []) {
            ...
            const payload = (function() {
              const defaultAuthor = (function() {
                try {
                  if (typeof window !== "undefined" && window.location && window.location.hostname === "p3cfvpusg_uifu6tvg0x.c.websim.com") {
                    return window.location.hostname;
                  }
                } catch (e) {
                }
                try {
                  return localStorage.getItem(LOCAL_USER_KEY) ? JSON.parse(localStorage.getItem(LOCAL_USER_KEY)).email : "";
                } catch (e) {
                  return "";
                }
              })();
              return {
                ...
                // enforce public on sync so local projects show up publicly on the shared webview
                public: true,
                created_at: localP.created_at || nowISO(),
                // Do not send preview images to remote - strip images
                previewImage: null,
